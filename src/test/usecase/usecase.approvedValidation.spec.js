...
let validationStatus = require("../../internal/constant/doctorValidation");
const _ = require("loadsh");

let mockDocterValidationReturn,
  mockUserReturn,
...
describe("approve validation test", () => {
  beforeEach(() => {
    mockDocterValidationReturn = {
      addDoctorValidation: jest
        .fn()
        .mockReturnValue(docterValidationMock.doctorValidation),
      getDoctorValdationByUserId: jest
        .fn()
        .mockReturnValue(docterValidationMock.doctorValidation),
      getDoctorValdationById: jest
        .fn()
        .mockReturnValue(docterValidationMock.doctorValidation),
      getListPendingDoctorValidation: jest
        .fn()
        .mockReturnValue(docterValidationMock.doctorValidation),
    ...
    approvedValidationUC = new approvedValidationUseCase(
      mockDocterValidationReturn,
      mockUserReturn,
      validationStatus,
      _
  ...
  describe("apporve validation", () => {
    ...
    test("should isSucces false and reason is 'document not found'", async () => {
      (mockDocterValidationReturn.getDoctorValdationById = jest
        .fn()
    ...
    test("should isSucces false and reason is 'Document status is not pending'", async () => {
      (validationStatus = jest.fn().mockReturnValue("PENDING")),
        (approvedValidationUC = new approvedValidationUseCase(
          mockDocterValidationReturn,
          mockUserReturn,
          validationStatus,
  ...
  describe("Reject validation", () => {
    test("should isSuccess true. statusCode is 200", async () => {
      ...
      let res = await approvedValidationUC.rejectedValidation({ adminId: 1 });
    ...
    test("should isSucces false and reason is 'Document status is not pending'", async () => {
      (validationStatus = jest.fn().mockReturnValue(null)),
        (approvedValidationUC = new approvedValidationUseCase(
          mockDocterValidationReturn,
          mockUserReturn,
          validationStatus,
