...
describe('product test', () => { 
    ...
    test("should isSuccess = true,statusCode = 200", async () => {
        ...
        expect(mockProductReturn.getProductById).toHaveBeenCalled();
        expect(mockProductReturn.deleteProduct).toHaveBeenCalled();
    ...
    describe('getProductById test', () => { 
        ...
        test("should isSuccess = true,statusCode = 200, and data is valid", async () => {
     ...
     describe('getListProduct test', () => { 
         
        test("should isSuccess = true, statusCode = 200, and data is true", async () => {
            let res = await productUC.getListProduct();

            expect(mockProductReturn.getListProduct).toHaveBeenCalled();
            ...
            expect(res.data[0]).toHaveProperty('categoryId');
            expect(res.data[0]).toHaveProperty('isStrip');
            expect(res.data[0]).toHaveProperty('price');
            expect(res.data[0]).toHaveProperty('description');
            expect(res.data[0]).toHaveProperty('indication');
            expect(res.data[0]).toHaveProperty('composition');
            expect(res.data[0]).toHaveProperty('dose');
            expect(res.data[0]).toHaveProperty('use');
            expect(res.data[0]).toHaveProperty('sideEffect');
            expect(res.data[0]).toHaveProperty('segmentation');
            expect(res.data[0]).toHaveProperty('packaging');
            expect(res.data[0]).toHaveProperty('manufacture');
            expect(res.data[0]).toHaveProperty('stock');
            expect(res.data[0]).toHaveProperty('urlImage');
            expect(res.data[0]).toHaveProperty('expiredAt');
        });

        test("should isSuccess = true, statusCode = 200, and data = [] ", async () => {
            mockProductReturn.getListProduct = jest.fn().mockReturnValue([]);
            productUC = new ProductUseCase(mockProductReturn, mockCategoryReturn, mediaHandler);
